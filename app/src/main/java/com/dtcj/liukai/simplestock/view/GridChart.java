package com.dtcj.liukai.simplestock.view;
...
import com.dtcj.liukai.simplestock.R;
...
public class GridChart extends View {
    ...
    public static final int DEFAULT_BACKGROUND = R.drawable.bg_chart;
    public static final int DEFAULT_AXIS_TITLE_SIZE = 22;   //默认XY轴字体的大小
    public static final int DEFAULT_AXIS_COLOR = Color.RED;    //默认XY坐标轴颜色
    public static final int DEFAULT_LONGI_LAITUDE_COLOR = Color.MAGENTA;   //默认经纬线颜色
    public static final int DEFAULT_UPER_LATITUDE_NUM = 3;     //默认上表纬线数
    public static final int DEFAULT_LOWER_LATITUDE_NUM = 1;    //默认下表纬线数
    public static final int DEFAULT_LOGITUDE_NUM = 3;   //默认经线数
    public static final int DEFAULT_BORDER_COLOR = Color.RED;   //默认边框的颜色
    private static final PathEffect DEFAULT_DASH_EFFECT = new DashPathEffect(new float[] { 3, 3, 3, 3 }, 1);    //默认虚线效果
    public static float LOWER_CHART_TOP;    //下表的顶部
    public static float UPER_CHART_BOTTOM;  //上表的底部

    /*************属性***************/
    private int mBackGround;    //背景色
    private int mAxisColor;     //坐标轴XY颜色
    private int mLongiLatitudeColor;    //经纬线颜色
    private PathEffect mDashEffect;     //虚线效果
    private int mBorderColor;       //边线色
    private float mUperChartHeight;     //上表高度
    private boolean showLowerChartTabs;     //是否显示下表Tabs
    private boolean showTopTitles;      //是否显示顶部Titles
    private float topTitleHeight;       //顶部Titles高度
    private String[] mLowerChartTabTitles;      //下表TabTitles
    private float mTabWidth;        //下表Tab宽度
    private float mTabHight;        //下表Tab高度
    private int mTabIndex;      //下表TabIndex
    ...
    public boolean onTouchEvent(MotionEvent event) {
        ...
        if (y <= LOWER_CHART_TOP + rect.top + 2 && y >= UPER_CHART_BOTTOM + DEFAULT_AXIS_TITLE_SIZE + rect.top) {
            if (mTabWidth <= 0) {
                return true;
            }
